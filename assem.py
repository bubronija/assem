...
def main():
    ...
    parser.add_argument('--test', action='store_true', help='запустить режим тестирования')
    ...
    try:
        ...
        print(f"кол-во ассемблированных команд: {cmd_count}")

        if args.test:
            all_hex = ", ".join([f"0x{b:02X}" for b in binary_output])
            print("\nполная последовательность байтов:")
            print(all_hex)
